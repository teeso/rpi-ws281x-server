...
function setRange(color, min, max) {
  ...
  var till = moment([2017]).utcOffset(-8, true).unix() - moment().unix();

  var segments = [];
  var bin = till;
  while(bin > 0) {
    segments.push(bin % 2 != 0);
    bin = bin >> 1;
  }
  ...
  segments.forEach(function(e,i) {
    var color = e ? rgb2Int(255,255,255) : rgb2Int(0,0,0);
    var size = Math.min(Math.floor(200 / segments.length),10);
    var start = i*size;
    var end = start + size;
    setRange(color, start, end);
  });

  if(till <= 0) {
    var color = (till % 2 == 0 || till < -60) ? rgb2Int(255,255,255) : rgb2Int(0,0,0);
    setRange(color, 0,200);
  }
/*
  if(count % 15 === 0) {
