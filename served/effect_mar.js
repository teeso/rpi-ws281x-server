...
exports.nextFrame = function(pixelData, count) {
  ...
  if( count % 600 === 100 ) {
   center2 = Math.floor(Math.random() * NUM_LEDS);
    size2 = 0;
  }

  if( count % 600 === 200 ) {
   center3 = Math.floor(Math.random() * NUM_LEDS);
    size3 = 0;
  }
  setRange(0,0,NUM_LEDS);


  var slow = 4;
  var left = Math.floor(center-size/slow);
  var right = Math.floor(center+size/slow);
  addRange(rgb2Int(Math.max(0,255-size),Math.max(0,255-size),0),left,right);
  size++;

  var slow = 4;
  var left = Math.floor(center2-size2/slow);
  var right = Math.floor(center2+size2/slow);
  addRange(rgb2Int(0,Math.max(0,255-size2),0),left,right);
  size2++;

  var slow = 4;
  var left = Math.floor(center3-size3/slow);
  var right = Math.floor(center3+size3/slow);
  addRange(rgb2Int(0,Math.max(0,255-size3),Math.max(0,255-size3)),left,right);
  size3++;
